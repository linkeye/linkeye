...
var (
	...
	configPrefix   = []byte("linkeye-config-") // config prefix for the db
...
func GetBlock(db DatabaseReader, hash common.Hash, number uint64) *types.Block {
	...
	block.DposContext = getDposContextTrie(db.(letdb.Database), header)
	return block
}

func getDposContextTrie(db letdb.Database, header *types.Header) *types.DposContext {
...
func WriteCanonicalHash(db letdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db letdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db letdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db letdb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db letdb.Putter, count uint64) error {
...
func WriteHeader(db letdb.Putter, header *types.Header) error {
...
func WriteBody(db letdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db letdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db letdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db letdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db letdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db letdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db letdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db letdb.Database) letdb.Database {
	return letdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db letdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db letdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db letdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
