...
const (
	defaultGasPrice = 50 * params.GuoSanBin
)
...
func (s *PublicAccountAPI) Accounts() []string {
	...
	for _, wallet := range s.am.Wallets() {
		for _, account := range wallet.Accounts() {
			addresses = append(addresses, Addr2LetStr(account.Address))
		}
	}
	return addresses
}
...
func (s *PrivateAccountAPI) NewAccount(password string) (string, error) {
	...
	if err == nil {
		return Addr2LetStr(acc.Address), nil
	}
	return Addr2LetStr(common.Address{}), err
}
...
//
// https://github.com/linkeye/linkeye/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with let_sign and personal_sign. As such it recovers
// the address of:
...
//
// https://github.com/linkeye/linkeye/wiki/Management-APIs#personal_ecRecover
...
func (s *PublicBlockChainAPI) rpcOutputBlock(b *types.Block, inclTx bool, fullTx bool) (map[string]interface{}, error) {
	...
	fields := map[string]interface{}{
		...
		"stateRoot":        head.Root,
		"signer":           Addr2LetStr(head.Coinbase),
...
func (args *SendTxArgs) setDefaults(ctx context.Context, b Backend) error {
	...
	maxlet := new(big.Int).Mul(onelet, big.NewInt(1))                   // 1LET

	feeValue := (*hexutil.Big)(args.Fee)
	if feeValue.ToInt().Cmp(minlet) < 0 {
		return errors.New(`fee < 0.01 LET`)
	}

	if feeValue.ToInt().Cmp(maxlet) > 0 {
		return errors.New(`fee > 1 LET`)
	}
	...
	//gasPrice is fixed, gasLimit = fee / gasPrice, fee ∈ [0.01,1] LET
...
//
// https://github.com/linkeye/wiki/wiki/JSON-RPC#let_sign
...
// Version returns the current linkeye protocol version.
