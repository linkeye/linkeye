...
import (
	...
	"github.com/linkeye/linkeye/internal/jsre"
	"github.com/linkeye/linkeye/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jlet", struct{}{})

	jethObj, _ := c.jsre.Get("jlet")
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jlet);"); err != nil {
	...
	flatten := "var let = web3.eth; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	if c.prompter != nil {
		...
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jlet.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jlet.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jlet.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jlet.sign = personal.sign;`); err != nil {
...
func (c *Console) Welcome() {
	// Print some generic Linkeye metadata
	fmt.Fprintf(c.printer, "Welcome to the Linkeye JavaScript console!\n\n")
	c.jsre.Run(`
