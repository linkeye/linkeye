...
import (
	...
	"github.com/linkeye/linkeye/signer/core"
	"github.com/linkeye/linkeye/signer/storage"
)
...
func (alwaysDenyUi) OnApprovedTx(tx letapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUi) OnApprovedTx(tx letapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(letapi.SignTransactionResult{})
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := letapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx letapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
