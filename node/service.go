...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (letdb.Database, error) {
	if ctx.config.DataDir == "" {
		return letdb.NewMemDatabase()
	}
	db, err := letdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
