...
func (n *Node) OpenDatabase(name string, cache, handles int) (letdb.Database, error) {
	if n.config.DataDir == "" {
		return letdb.NewMemDatabase()
	}
	return letdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
